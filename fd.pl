...
$a = sprintf "%f", $trios_len / $processor;
$a = int $a;
...
for $temp_trios (@trios2) {
	...
	for $trio (@trios) {
		...
		$fd_total = $denominator_fd_total ? ($numerator_d_total / $denominator_fd_total) : 'NA';
...
sub print_window_d_fd {
	for $key (keys %ABBA) {
		...
		$fd_window = $denominator_fd_window ? ($numerator_d_window / $denominator_fd_window) : 'NA';
