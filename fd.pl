...
$help = <<"EOF";
	Calculate D (ABBA-BABA) and fd statistics for sliding window from VCF file.
	Usage: perl $0 [options]
		-v   vcf file
		-t   trios file
		-m   map flie. It should contain two columns, first is sample name, second is population name. Name of outgroup population must be 'Outgroup'.
		-i   interval file. Three columns, first is chromosome name, second is 1-based start coordinate, third is 1-based end coordinate.
...
### open vcf file and get sample names
...
### and output D and fd of windows for every trio
%ABBA_total = ();
%BABA_total = ();
%denominator_d_total = ();
